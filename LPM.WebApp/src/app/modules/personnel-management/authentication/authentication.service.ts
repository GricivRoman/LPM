...
export class AuthenticationService {
	...
	get loginRequired(): boolean {
		return this.AuthLocalStorageService.authInfo == null || new Date(this.AuthLocalStorageService.authInfo.expiration) < new Date(Date.now());
	}
	...
	private loginRequest(model: LoginModel, form?: FormGroup): Observable<any>{
		return this.http.post('account/login', model).pipe(map((response) => {
			this.AuthLocalStorageService.authInfo = response as AuthResponse;
